...
package {
	...
	public class DashedLine extends Shape {
		...
		private var doDash:Boolean = false;
		private var currDist:Number = 0;
		public function DashedLine(weight:Number = 0, color:Number = 0, lengthsArray:Array = null){
			...
			stroke.graphics.lineStyle(lineWeight,lineColor,lineAlpha);
		...
		public function lineTo(x:Number,y:Number):void{
			...
			outerLoop : while ((Math.abs(startX-curX) < Math.abs(startX-x) || Math.abs(startY-curY) < Math.abs(startY-y)) && stuckCounter < 1000){stuckCounter++;
				...
				for (var i:int = startIndex; i < lengthsArray.length; i++){
					...
					if (Math.abs(startX-curX) + Math.abs(xInc) < Math.abs(startX-x) || Math.abs(startY-curY) + Math.abs(yInc) < Math.abs(startY-y)){
						if (doDash){	// if even index in the array, it is a dash, hence lineTo
			...
			if (remainingDist != 0){	// if there is a remaining distance, line or move from current x/y to the destination x/y
				currDist += remainingDist;
				if(currDist > lengthsArray[curIndex])
				{
					doDash = !doDash;
					currDist -= lengthsArray[curIndex];
				}
				if (doDash){	// even = dash
